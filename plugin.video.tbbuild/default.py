...
addon_id = 'plugin.video.tbbuild'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.video.tbbuild'
AddonTitle="Tb"
...
PATH = "The"            
BASEURL = "http://www.tbbuild.co.uk"
H = 'http://'
...
def BUILDMENU():
    addDir('INSTALL TB BUILD',BASEURL+'/build/backup.zip',5,ART+'install.png',FANART,'')
    setView('movies', 'MAIN')
...
def WIZARD(name,url,description):
    ...
    dp.create("TB BUILD","Downloading ",'', 'Please Wait')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("TB BUILD Wizard", "To save changes you now need to force close Kodi, Press OK to force close Kodi")
    killxbmc()
        ...
        # Count files and give option to delete
            if file_count > 0:
                ...
                if dialog.yesno("Delete Package Cache Files", str(file_count) + " files found", "Do you want to delete them?"):
                    ...
                    dialog = xbmcgui.Dialog()
                    dialog.ok("Packages Successfuly Removed", "[COLOR yellow]TB BUILD[/COLOR]")
    except: 
        dialog = xbmcgui.Dialog()
        dialog.ok("Sorry we were not able to remove Package Files", "[COLOR yellow]TB BUILD[/COLOR]")
...
def deletecachefiles(url):
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok(" All Cache Files Removed", "[COLOR yellow]TB BUILD[/COLOR]")
...
def killxbmc():
    ...
    if myplatform == 'osx': # OSX
        ...
        dialog.ok("[COLOR=red][B]WARNING  !!![/COLOR][/B]", "Your system has been detected as Android, you ", "[COLOR=yellow][B]MUST[/COLOR][/B] force close XBMC/Kodi. [COLOR=lime]DO NOT[/COLOR] exit cleanly via the menu.","Pulling the power cable is the simplest method to force close, or exit via Task Manager.")
